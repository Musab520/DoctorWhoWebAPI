...
{
    public class DoctorForInsertDtoValidator : AbstractValidator<DoctorForInsertDto>
    {
        public DoctorForInsertDtoValidator()
        {
            RuleFor(doctor => doctor.DoctorNumber).NotEmpty().NotEqual(0).WithMessage("Doctor Number Cannot be Empty");
